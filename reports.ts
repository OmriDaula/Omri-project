...
function safeGetExpensesReports(): Expense[] {
...
function formatCurrency(amount: number): string {
    if (typeof (window as any).formatILS === 'function') return (window as any).formatILS(amount);
...
function sumByYear(expenses: Expense[]): Record<string, number> {
    const map: Record<string, number> = {};
...
function sumByMonth(expenses: Expense[]): Record<string, number> {
    const map: Record<string, number> = {};
    for (const e of expenses) {
...
function updateStats(expenses: Expense[]): void {
...
function renderPie(expenses: Expense[]): void {
    const byCat = sumByCategory(expenses);
...
function renderBar(expenses: Expense[]): void {
    const groupBy = (document.getElementById('groupBy') as HTMLSelectElement)?.value || 'month';
...
function renderAll(): void {
    const expenses = safeGetExpensesReports();
