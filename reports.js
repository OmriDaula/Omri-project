"use strict";
function safeGetExpensesReports() {
...
function formatCurrency(amount) {
    if (typeof window.formatILS === 'function')
        return window.formatILS(amount);
...
function updateStats(expenses) {
    ...
    if (statCount)
        statCount.textContent = `${count} רשומות`;
    if (statAvg)
        statAvg.textContent = formatCurrency(avg);
    if (statTopCat)
        statTopCat.textContent = topCat;
    if (statTopCatSum)
        statTopCatSum.textContent = formatCurrency(topSum);
}
